...
eval(src.slice(src.indexOf('function colStudent_'), src.indexOf('//  설정 읽기'))
...
// ── 열 위치 ──
t('1번 학생은 B열(2)', () => a.equal(colStudent_(1), 2));
t('25번 학생은 26열', () => a.equal(colStudent_(25), 26));
...
t('그룹평균 열은 학생 열 바로 뒤', () => a.equal(colAvg_(25, 1), 27));
t('잠금 열은 고정 위치 (학생 25명 → 37열)', () => a.equal(colLock_(25), 37));
t('그룹 수가 바뀌어도 잠금 열이 안 밀린다', () => {
  // 이게 안 지켜지면 그룹을 바꿀 때 체크박스와 점수가 어긋난다
  a.equal(colLock_(25), colLock_(25));
  for (const g of [3, 4, 5, 6]) {
    a.equal(colLock_(25), 1 + 25 + MAX_GROUPS + 1, '그룹당 ' + g + '명일 때');
  }
});
t('그룹평균 10칸이 잠금 열과 안 겹친다', () => {
  a.ok(colAvg_(25, MAX_GROUPS) < colLock_(25));
});
t('열이 겹치지 않는다', () => {
...
t('columnLetter_ 경계값', () => {
  ...
  a.equal(columnLetter_(27), 'AA'); a.equal(columnLetter_(37), 'AK');
});
